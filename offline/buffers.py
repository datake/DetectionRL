...
class BatchBuffer():
    ...
    def get_minibatch(self,index,sample_size,action_space,state_space,states,actions,returns,values,log_probs,advs):
        batch_states = np.reshape(states[:, index:index + sample_size], [-1] + state_space)
        # batch_actions = np.reshape(actions[:, index:index + sample_size], [-1])
        batch_actions = np.reshape(actions[:, index:index + sample_size], [-1] + action_space)
        # print('batch_actions :',batch_actions.shape)
        # batch_actions = np.reshape(actions[:, index:index + sample_size], [-1,1])
        # batch_returns = np.reshape(returns[:, index:index + sample_size], [-1])
        batch_returns = np.reshape(returns[:, index:index + sample_size], [-1, 1])
        # batch_values = np.reshape(values[:, index:index + sample_size], [-1])
        batch_values = np.reshape(values[:, index:index + sample_size], [-1, 1])
        batch_log_probs = np.reshape(log_probs[:, index:index + sample_size], [-1])
        # batch_log_probs = np.reshape(log_probs[:, index:index + sample_size], [-1,1])
        # batch_advs = np.reshape(advs[:, index:index + sample_size], [-1])
