...
class nature_cnn(nn.Module):
    def __init__(self,action_space,state_space):
        ...
        nn.init.orthogonal_(self.conv1.weight.data,gain=np.sqrt(2.))
        nn.init.orthogonal_(self.conv2.weight.data,gain=np.sqrt(2.))
        nn.init.orthogonal_(self.conv3.weight.data,gain=np.sqrt(2.))
        ...
        nn.init.orthogonal_(self.fc.weight.data,gain=np.sqrt(2.))
        nn.init.constant_(self.fc.bias.data,0.0)
        nn.init.orthogonal_(self.logits.weight.data,gain=np.sqrt(2.))
    ...
    def forward(self,unscaled_images):
        ...
        s = unscaled_images # scale, [8, 84, 84, 4] in [0, 1]
        ...
        s = nn.functional.relu(self.fc(s)) # 512 feature map/vector
        logits = self.logits(s)

        p = torch.nn.Softmax(dim=-1)(logits) + 1e-8
        # p = torch.nn.Softmax(dim=-1)(logits)
        policy_head = Categorical(probs=p) # to sample an action from the policy distribution
        ...
        # output 3 quantities from the feature map s
        return policy_head,value,s